        ...
        <div className="ds-section-spacing animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
                ...
                <Card className="lg:col-span-2 bg-white rounded-xl p-4 border-none shadow-xl ring-1 ring-slate-100 overflow-hidden relative">
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-4 relative z-10">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-6">
                            <div>
                                <h3 className="text-3xl font-semibold text-slate-900">{customer.name}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                {[
                                    ...
                                    <div key={i} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-center justify-center text-center">
                                        ...
                                        <h5 className="text-lg font-semibold text-slate-900 mt-1">{stat.value}</h5>
                ...
                <div className="space-y-4">
                    <Card className="p-6 !bg-black  text-primary-foreground rounded-xl border-none shadow-lg shadow-brand-200 relative overflow-hidden group">
                        <div className="relative z-10">
                            <p className="text-[10px] font-semibold opacity-90 uppercase tracking-widest mb-1">Lifetime Value</p>
                            <h4 className="text-3xl font-semibold text-white">₹{(customer.totalSpent || 0).toLocaleString()}</h4>
                            <div className="mt-4 flex items-center gap-2">
                                <div className="p-1 px-2 rounded-full bg-white/25 text-white text-[10px] font-semibold uppercase tracking-tighter">
                    ...
                    <Card className="p-6 bg-white rounded-xl border-none shadow-md ring-1 ring-slate-100">
                        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Recent Activity</p>
                        <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    ...
                                    return (
                                        <div key={addr._id || addr.id || idx} className={cn(
                                            ...
                                            <div className="flex items-center justify-between mb-2">
                                                <Badge variant={isDefault ? 'primary' : 'secondary'} className="text-[9px] font-semibold">
                                                    {type}
                                                </Badge>
                        ...
                        <div className="p-4 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="text-[10px] font-semibold text-brand-600 uppercase hover:underline"
                                >
                                    Export CSV
                                </button>
                            </div>
                                    ...
                                    {filteredOrders.map((order, i) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-5 text-center">
                                                <Badge variant={order.status === 'delivered' ? 'success' : order.status === 'cancelled' ? 'danger' : 'warning'} className="text-[8px] font-semibold">
                                                    {order.status.toUpperCase()}
                                                </Badge>
                                            </td>
                                            <td className="py-5 text-right font-semibold text-slate-900 pr-8">
                        ...
                        {visibleOrders < safeOrders.length && (
                            <div className="p-4 bg-slate-50/50 flex justify-center border-t border-slate-50">
                                <button
                                    ...
                                    className="text-[10px] font-semibold text-brand-600 uppercase hover:underline flex items-center gap-2"
                                >
                                    SHOW ALL ORDERS
                ...
                <div className="space-y-6">
                    <Card className="border-none shadow-xl ring-1 ring-slate-100 bg-white rounded-xl p-4">
                        <h4 className="text-xs font-semibold text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="w-full mt-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl text-[10px] font-semibold uppercase tracking-widest transition-all"
                        >
                            UPDATE NOTES
                        </button>
                    </Card>
                    ...
                    <Card className="border-none shadow-xl ring-1 ring-slate-100 bg-slate-900 rounded-xl p-4 text-white">
                        <h4 className="text-xs font-semibold opacity-40 uppercase tracking-widest mb-6">Account Control</h4>
                        <div className="space-y-4">
                            <button
                                onClick={() => setIsNotifModalOpen(true)}
                                className="w-full py-4 bg-black  hover:bg-brand-500 text-primary-foreground rounded-2xl font-semibold text-[11px] uppercase tracking-widest transition-all shadow-xl shadow-brand-900/20 flex items-center justify-center gap-2"
                            >
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white rounded-2xl font-semibold text-[11px] uppercase tracking-widest border border-rose-500/20 transition-all flex items-center justify-center gap-2"
                            >
            ...
            <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Profile Details">
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                    ...
                    <button type="submit" className="w-full py-4 bg-black  hover:bg-brand-500 text-primary-foreground rounded-2xl font-semibold text-[11px] uppercase tracking-widest transition-all shadow-lg active:scale-95">
                        SAVE CHANGES
                    </button>
                </form>
            </Modal>
            ...
            <Modal isOpen={isNotifModalOpen} onClose={() => setIsNotifModalOpen(false)} title="Send Notification">
                <div className="space-y-6">
                    ...
                    <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest block mb-2">Message</label>
                        <textarea
                            value={notifMessage}
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-slate-900 text-white rounded-2xl font-semibold text-[11px] uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-50"
                    >
                        SEND MESSAGE
                    </button>
                </div>
            ...
            <Modal isOpen={isRestrictModalOpen} onClose={() => setIsRestrictModalOpen(false)} title="Confirm Action">
                <div className="space-y-6">
                    <div className="p-6 bg-rose-50 rounded-xl border border-rose-100 flex flex-col items-center text-center gap-4">
                        ...
                        <h5 className="text-lg font-semibold text-slate-900 uppercase tracking-tight">
                    ...
                    <div className="flex gap-3">
                        <button onClick={() => setIsRestrictModalOpen(false)} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-semibold text-[11px] uppercase tracking-widest hover:bg-slate-200 transition-all">
                            CANCEL
                        </button>
                        <button onClick={handleRestrictAccount} className="flex-1 py-4 bg-rose-500 text-white rounded-2xl font-semibold text-[11px] uppercase tracking-widest hover:bg-rose-600 shadow-xl shadow-rose-200 transition-all">
                            CONFIRM
                        </button>
                    </div>
                </div>
